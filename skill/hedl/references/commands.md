...
Steps:
1. **Choose depth** — Standard (security-auditor, new-engineer, quality-synthesizer) or Full (all 8 core agents + composable reviewers from review-library.md). Use Standard for routine checks; Full for post-major-change or phase transitions.
...
3. **Convene panel** — instantiate agents from `.claude/agents/` (core 8) and from `review-library.md` (for Full depth).
