...
> - Before any agent runs, the `00-baseline-facts.md` must include an ADR-survey: the project's stated architectural intent for each pivotal axis (target harnesses, target deployment surfaces, target AI tools). The native-redundancy and competitive-landscape agents must compare against that intent, not against the today-observed single-harness surface, unless the design intent itself is also reviewed for plausibility.
>
